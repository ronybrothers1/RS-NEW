...
export type DonationPushDeliveryResult = {
  enabled: boolean;
  available: boolean;
...
function logDonationDeliveryFailure(
  operation: string,
  error: unknown,
) {
  const message =
...
export async function deliverPushToDonationBestEffort(
  donationId: string,
  message: SafePushMessage,
): Promise<DonationPushDeliveryResult> {
  if (
    !notificationsEnabled()
  ) {
    return {
  ...
  try {
    ...
    logDonationDeliveryFailure(
      "load_donation_subscriptions",
      error,
    );
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          logDonationDeliveryFailure(
            "deactivate_gone_donation_subscription",
            error,
          );
        }
