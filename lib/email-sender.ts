...
export async function sendPasswordResetEmail({
    ...
    await fetch(
      "https://api.resend.com/emails",
      {
        ...
        body: JSON.stringify({
          ...
          to: [to],
          subject:
            "Kode Pemulihan Password Ruang Sejahtera",
          html: `
            <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;color:#0f172a">
              <h2>Pemulihan Password</h2>
              <p>Kami menerima permintaan untuk mengatur ulang password akun Ruang Sejahtera.</p>
              <p>Gunakan kode berikut:</p>
              <div style="font-size:32px;font-weight:700;letter-spacing:8px;margin:24px 0">
                ${escapeHtml(code)}
              </div>
              <p>Kode berlaku selama 10 menit.</p>
              <p>Jika Anda tidak meminta pemulihan password, abaikan email ini.</p>
            </div>
          `,
        }),
      },
  ...
  if (!response.ok) {
    ...
    console.error(
      "Resend password reset API error:",
      response.status,
      detail,
    );

    throw new Error(
      "Email pemulihan password gagal dikirim.",
    );
  }
}